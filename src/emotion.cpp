...
int EmotionClassifier::m_classCount = 0;
...
(Logger gLogger, DataType dtype, const string uffFile, const string engineFile, int batchSize, bool serializeEngine,
    ...
    m_INPUT_C = static_cast<const int>(1);
    m_INPUT_H = static_cast<const int>(64);
    m_INPUT_W = static_cast<const int>(64);
...
void EmotionClassifier::createOrLoadEngine() {
    if(fileExists(m_engineFile)) {
        ...
        parser->registerInput("input_1", DimsCHW(m_INPUT_H, m_INPUT_W, m_INPUT_C), UffInputOrder::kNHWC);
        parser->registerOutput("predictions/Softmax");
...
void EmotionClassifier::getCroppedFacesAndAlign(cv::Mat frame, std::vector<struct Bbox> outputBbox) {
    for(vector<struct Bbox>::iterator it=outputBbox.begin(); it!=outputBbox.end();it++){
        if((*it).exist){
            ...
            cv::resize(tempCrop, currFace.faceMat, cv::Size(m_INPUT_H, m_INPUT_W), 0, 0, cv::INTER_CUBIC);
            currFace.x1 = it->x1;
            currFace.y1 = it->y1;
            currFace.x2 = it->x2;
            currFace.y2 = it->y2;
